...
public final class Log {
    ...
    public struct Message {
        ...
        public var formatted: String {
            return "\(Date.now.formatted(.dateTime.hour(.twoDigits(amPM: .omitted)).minute().second())) \(level.prefix) | [\(filename):\(line)] | \(message.flatMap { $0 as? [Any] ?? [$0] }.map { String(describing: $0) } .joined(separator: " "))"
        }
    }
}
